# used to track visits to the main recommendations page
class Analytics::EngagedVisitor < Analytics::AnalyticsBase
	def track(options={})
		if options[:b] 
				button_key = create_key('button:' + Date.today.to_s)
				$redis.zincrby button_key, 1, options[:user_id]
